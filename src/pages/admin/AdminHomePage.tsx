...
export default function AdminHomePage() {
  ...
  const myBoothPath =
    profile?.role === 'ADMIN' && profile.boothId
  ...
  const ADMIN_MENUS = [
    ...
    { id: 'booth', label: '내 부스', icon: FiHome, path: myBoothPath },
      ...
      <div className="grid w-full max-w-md grid-cols-2 gap-4 px-4">
        {ADMIN_MENUS.map((menu) => (
          <button
            ...
            className="group flex aspect-square flex-col items-center justify-center rounded-[32px] border border-gray-100 bg-white px-2 py-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-knu-red/40 hover:shadow-md active:scale-95"
          >
